...
def path := "/home/sk/lean-projects/csvParse/ljpCSV.csv"

def input (path : String) : IO String :=
...
-/
def manySep {α β : Type} (p : Parsec α) (s : Parsec β) : Parsec $ Array α := do
  manyCore (attempt (s *> p)) #[←p]

def record : Parsec Record :=
  manySep field comma
...
-/
partial def manyHomoCore {α : Type} (p : Parsec $ Array α) (acc : Array $ Array α) : Parsec $ Array $ Array α :=
  (do
    let first ← p
...
-/
def manySepHomo {α β : Type} (p : Parsec $ Array α) (s : Parsec β) : Parsec $  Array $ Array α := do
...
def parsed_val := do
  let i <- input path
  match parse i with
  | Except.ok res => res.map (Array.toList ·) |>.toList |> pure
  | Except.error _ =>  pure []
